...
class SpectrumTestController extends Controller
{
    public function index()
    {
        return 0;
        $order = Order::find(8213);
        ...
        // return $cobDataFinal;
            ...
            return $data;
            // $client = new Client();
            // $res = $client->post(env('SPECTRUM_URL_API_STAGGING'), 
            // [
            //     // 'verify' => false,
    ...
    function rehitSpectrum($id) {
        // dispatch(new SpectrumJob(8213));
        return "masuk";
        $transactions = Transaction::where('trx_data', '!=', null)
            ->where('id', '>=', 5693)
            ->where('contract_id', '=', null)
            ->with(['cart' => function ($query) {
                $query->where('pg_status', '2');
            }])
            ->get();
        return $transactions;
        // foreach ($transactions as $trx_key => $transaction) {
        //         Log::alert("SENDING REHIT DATA TO SPECTRUM");
        //         dispatch(new SpectrumJob($transaction->order->id));
        //         Log::alert("SENDING REHIT DATA TO SPECTRUM SUCCESSFULLY");
        //     }
        // return response([
        //         "status" => true,
        //         "message" => "REHIT TO SPECTRUM SUCCESSFULLY!!!",
        // ], 404);

        
        // $cart = Cart::where('pg_status', $id)->first();
        // if(empty($cart)) {
        //     return response([
        //         "status" => false,
